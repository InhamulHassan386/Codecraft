...
app.put("/api/admins/:id", async (req, res) => { const { name, email, role, isActive, profileImage } = req.body; try { await pool.execute("UPDATE admin_users SET name=?, email=?, role=?, is_active=?, profile_image=? WHERE id=?", [name,email,role,isActive === false ? 0 : 1,profileImage || null,req.params.id]); const [rows] = await pool.query("SELECT id,name,email,role,is_active AS isActive,last_login AS lastLogin,profile_image AS profileImage FROM admin_users WHERE id=?", [req.params.id]); await logActivity(req.params.id, "Admin updated", `Updated ${email}`); res.json(rows[0]); } catch { res.status(400).json({ error: "Could not update admin" }); } });
app.delete("/api/admins/:id", async (req, res) => { try { const [result] = await pool.execute("DELETE FROM admin_users WHERE id=?", [req.params.id]); if (!result.affectedRows) return res.status(404).json({ error: "Admin not found" }); await logActivity(req.params.id, "Admin deleted"); res.status(204).end(); } catch { res.status(400).json({ error: "Could not delete admin" }); } });
app.post("/api/admins/:id/password", async (req, res) => { const { currentPassword, password } = req.body; if (!password || password.length < 8) return res.status(400).json({ error: "Password must be at least 8 characters" }); try { const [rows] = await pool.query("SELECT password_hash FROM admin_users WHERE id=?", [req.params.id]); if (!rows[0] || !(await bcrypt.compare(currentPassword || "", rows[0].password_hash))) return res.status(401).json({ error: "Current password is incorrect" }); const hash = await bcrypt.hash(password, 12); await pool.execute("UPDATE admin_users SET password_hash=? WHERE id=?", [hash, req.params.id]); await logActivity(req.params.id, "Password changed"); res.json({ ok: true }); } catch { res.status(400).json({ error: "Could not change password" }); } });
