...
export default function AdminManagement() {
  ...
  const openPermissions = async (admin: Admin) => { const response = await authFetch(`/api/admins/${admin.id}/permissions`); setPermissions(response.ok ? await response.json() : []); setPermissionAdmin(admin); };
  const savePermissions = async () => { if (!permissionAdmin) return; await authFetch(`/api/admins/${permissionAdmin.id}/permissions`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ permissions }) }); setPermissionAdmin(null); };

  const loadAdmins = async () => {
    try {
      const response = await authFetch("/api/admins");
      if (!response.ok) throw new Error();
      const rows = await response.json(); setAdmins(rows); if (rows[0]) { const profileResponse = await authFetch(`/api/admins/${rows[0].id}/profile`); if (profileResponse.ok) setProfile(await profileResponse.json()); }
    } catch { setError("Admins load nahi ho sake. API aur database check karein."); }
    finally { setLoading(false); }
  };

  useEffect(() => { void loadAdmins(); authFetch("/api/activity-logs").then((r) => r.ok ? r.json() : []).then(setLogs); }, []);

  const updateAdmin = async (admin: Admin, changes: Partial<Admin>) => {
    const response = await authFetch(`/api/admins/${admin.id}`, {
      method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(changes),
    });
    if (response.ok) { const updated = await response.json(); setProfile(updated); await loadAdmins(); } else setError("Admin update nahi ho saka.");
  };

  const deleteAdmin = async (admin: Admin) => {
    if (!window.confirm(`${admin.name} ko delete karna hai?`)) return;
    const response = await authFetch(`/api/admins/${admin.id}`, { method: "DELETE" });
